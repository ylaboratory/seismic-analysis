...
ts_obj_symbol = seismicGWAS::munge_sce_mat(ts_obj, rowData(ts_obj) %>% as_tibble(rownames = "ensembl") %>% select(ensembl, gene_symbol))

###subset data set
all_k = c(300000, 250000, 200000, 150000,100000, 50000,25000, 10000)
for (i in 1:5){
  for (k in all_k){
    ...
    save(ts.sce, file = here("data","expr","runtime","expr_rda",paste0("ds_",i), paste0("ts.sce.",k/1000,"k.rda") ) )
    #sce for anndata
    ts.adata = ts_obj_symbol[,seed_sample]
    ts.adata = SCE2AnnData(ts.adata)
    ts.adata %>% write_h5ad(here("data","expr","runtime","expr_h5ad",paste0("ds_",i), paste0("ts.",k/1000,"k.h5ad")))
  }
}
