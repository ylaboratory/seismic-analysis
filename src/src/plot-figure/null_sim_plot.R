...
null_res_expr_sum <- map(c("seismic","scDRS", "S-MAGMA"), ~select(null_res_expr_all, all_of(c("index","ds",.x)))) %>% #summary info
  set_names(c("seismic","scDRS", "S-MAGMA")) %>%
...
null_res_expr_fuma_sum <- null_res_expr_all %>%
  select(index, ds, FUMA) %>%
  drop_na() %>%
  split(.$ds) %>% #seperate by data set size
  map(~arrange(.x, FUMA)) %>%
  map(~add_count(.x)) %>%
  map(~mutate(.x, index=1:n())) %>%
  map(~dplyr::slice(.x, floor(unique(.x$n)/10^ seq(0,3,0.1)))) %>%
  map(~mutate(.x,  exp_q = -log10(index/n))) %>%
  map(~mutate(.x, around_q = seq(0,3,0.1))) %>%
  purrr::reduce(~rbind(.x, .y)) %>% 
  group_by(around_q) %>%
  mutate(exp_q = mean(exp_q), mean = mean(-log10(FUMA)), sd = sd(-log10(FUMA)), mean_max = mean+sd, mean_min = max(0, mean-sd) ) %>%
  ungroup() %>%
  distinct(mean, exp_q, mean_max, mean_min) %>%
  mutate(method="FUMA")

null_res_expr_sum <- rbind(null_res_expr_sum, null_res_expr_fuma_sum) %>%
  mutate(framework = method) %>%
...
null_res_gs_sum <- map(c("seismic","scDRS"), ~select(null_res_gs_all, all_of(c("index","ds",.x)))) %>% #summary info
  ...
  map(~mutate(.x,mean_max = mean+sd)) %>% 
  map(~mutate(.x,mean_min = mean-sd)) %>%
  map(~select(.x,all_of(c("mean","exp_q", "mean_max", "mean_min")))) %>%
  map2(names(.), ~mutate(.x, method = .y)) %>%
  purrr::reduce(~rbind(.x,.y)) %>% #combine together
  rowwise() %>%
  mutate(mean_min = max(0,mean_min)) %>%
