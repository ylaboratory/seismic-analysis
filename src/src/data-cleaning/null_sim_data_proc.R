...
facs_obj_sce <- facs_obj_sce[,which(!is.na(facs_obj_sce$cell_ontology_class))]
...
for (i in 1:10){
  ...
  sce <- SingleCellExperiment::SingleCellExperiment(list(counts = facs.count_mat[gene_idx,cell_idx] ,
                                                        logcounts = facs.adjusted.log[gene_idx,cell_idx] , 
...
for (i in 1:10){
  load(paste0("data/expr/null_sim/expr_rda_rs/expr_ds_",i,".rda"))
  cell = sce$cellid %>% gsub(pattern="cell_",replacement = "", x=.) %>% as.numeric 
  size_factors = facs_obj_sce$sizeFactor[cell]
  count_mat = as(2^assay(sce, "logcounts")-1, "dgCMatrix")  %>% sweep_sparse(margin = 2, stats = size_factors, fun="*") %>% round()
  assay(sce, "counts") = count_mat
  save(sce, file=paste0("data/expr/null_sim/expr_rda_rs/expr_ds_",i,".rda"))
}
