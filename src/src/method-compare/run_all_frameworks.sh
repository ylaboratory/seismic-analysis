...
nohup bash src/tools/FUMA_MAGMA_batch.sh -m MAGMA -e data/expr/Tabula_sapiens/ts.top10_magma.txt -g data/gwas/tm_gwas/magma_raw -o results/Tabula_sapiens/S-MAGMA &
nohup bash src/tools/FUMA_MAGMA_batch.sh -m FUMA -e data/expr/Tabula_sapiens/ts.fuma.txt -g data/gwas/tm_gwas/magma_raw -o results/Tabula_sapiens/FUMA &
...
for f in $all_gs_files; do
    g_prefix=$(basename "$f" | cut -d'.' -f1)
    python bin/scdrs compute-score data/expr/Tabula_sapiens/ts.clean.h5ad human "$f" human results/Tabula_sapiens/scDRS --flag-raw-count True
    python bin/scdrs perform-downstream --h5ad-file data/expr/Tabula_sapiens/ts.clean.h5ad --score-file results/Tabula_sapiens/scDRS/"$g_prefix".full_score.gz\
    --group-analysis "cluster_name" --out-folder results/Tabula_sapiens/scDRS --flag_raw_count True
done
...
for g in "${granularity[@]}"; do
    nohup bash src/tools/FUMA_MAGMA_batch.sh -m MAGMA -e data/expr/Saunders/Saunders."$g".top10_magma.txt -g data/gwas/brain_gwas/magma_raw -o results/Saunders/"$g"/S-MAGMA &
done
