...
ts_obj = munge_sce_mat(ts_obj, gene_anno %>% select(ensemblid,ensembl)) #merge by ensemblid (mean)

#add something to the colData
rowData(ts_obj) = rowData(ts_obj) %>% as_tibble() %>% 
  left_join(gene_anno %>% select(ensembl, gene_symbol) %>% distinct(ensembl,.keep_all=TRUE),by="ensembl") %>%
  DataFrame() %>%
