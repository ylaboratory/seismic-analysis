...
library(cowplot)
color_mapping_vec_other <- c("scDRS" ="#46B8DAFF", "FUMA"="#5CB85CFF", "S-MAGMA"="#EEA236FF" )
res_sup <- list("scDRS"= res_scdrs$fine_cluster, "FUMA" = res_fuma$fine_cluster, "S-MAGMA" = res_magma$fine_cluster) 

res_sup_clin <- res_sup %>%
  map(~select(.x, cell_type, Kunkleetal_2019)) %>%
  map(~rename(.x, "pvalue" = "Kunkleetal_2019")) %>%
  map(~mutate(.x, FDR = p.adjust(pvalue, method = "fdr")))

res_sup_tau <- res_sup %>%
  map(~select(.x, cell_type, tau)) %>%
  map(~rename(.x, "pvalue" = "tau")) %>%
  map(~mutate(.x, FDR = p.adjust(pvalue, method = "fdr")))

plot_clin <- res_sup_clin %>% 
  map(~seismicGWAS::plot_top_associations(.x) + 
        geom_hline(aes(yintercept = -log10(0.05)), linetype = "dashed") +
        ylim(0,3.5) + 
        theme(axis.text.x = element_text(size = 8))) %>%
  map2(color_mapping_vec_other, ~.x + geom_bar(fill =  .y, stat = "identity")  )

plot_tau <- res_sup_tau %>% 
  map(~seismicGWAS::plot_top_associations(.x) + 
        geom_hline(aes(yintercept = -log10(0.05)), linetype = "dashed") +
        ylim(0,3.5) + 
        theme(axis.text.x = element_text(size = 8))) %>%
...
col1 <- plot_grid(plotlist = plot_clin, ncol = 1)
col2 <- plot_grid(plotlist = plot_tau, ncol = 1)
