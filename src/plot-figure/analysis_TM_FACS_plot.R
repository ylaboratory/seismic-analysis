...
#upset plot
all_pair <- unique(c(facs_seismic_pair, facs_scdrs_pair, facs_fuma_pair, facs_magma_pair))
upset_data <- data.frame(seismic = ifelse( all_pair %in% facs_seismic_pair, 1, 0),
                         scDRS = ifelse( all_pair %in% facs_scdrs_pair, 1,0),
                         FUMA = ifelse(all_pair %in% facs_fuma_pair, 1,0),
                         `S-MAGMA` = ifelse( all_pair %in% facs_magma_pair, 1, 0),
                         check.names = F) %>%
  set_rownames(all_pair)

set_colors <- c(
  "seismic" = adjustcolor("#D43F3A", alpha.f = 0.8),
  "scDRS" = adjustcolor("#46B8DA", alpha.f = 0.8),
  "FUMA" = adjustcolor("#5CB85C", alpha.f = 0.8),
  "S-MAGMA" = adjustcolor("#EEA236", alpha.f = 0.8)
)

upset(
  upset_data,
  sets = rev(c("seismic", "scDRS", "FUMA", "S-MAGMA")),
  keep.order = TRUE,
  order.by = "degree",
  decreasing = TRUE,
  mainbar.y.label = "Number of intersected cell type-trait pairs",
  sets.x.label = "Total identified cell type-trait pairs",
  text.scale = 1.2,
  point.size = 3,
  line.size = 0,
  mb.ratio = c(0.6, 0.4),
  number.angles = 0,
  main.bar.color = adjustcolor("#3355AA", alpha.f = 0.8),
