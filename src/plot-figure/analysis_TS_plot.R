...
#load results - ts
ts_res <- read.table( here("results","Tabula_sapiens","seismic","ts_res.new.txt"), header = T, sep = "\t") %>%
  as_tibble() %>%
...
ts_scdrs_res <- list.files(here("results","Tabula_sapiens","scDRS_new"),pattern = "*.scdrs_group.cluster_name",full.names = T) %>% 
...
ts_fuma_res <- list.files(here("results","Tabula_sapiens","FUMA_new"), pattern = "*.gsa.out", full.names = T) %>%
...
ts_magma_res <- list.files(here("results","Tabula_sapiens","S-MAGMA_new"), pattern = "*.gsa.out", full.names = T) %>%
  set_names(str_extract(string = . , pattern = "(?<=/)[^/]+$") %>% gsub(pattern = "\\.[A-Za-z.]+$", replacement = "", x= .)) %>%
  map(~read.table(.x, header = T) %>% as_tibble()) %>%
  map(~select(.x, any_of(c("VARIABLE", "P")))) %>%
  map2(names(.), ~set_colnames(.x, c("cell_type", .y))) %>%
...
ts_fuma_ct_mapping <- read.table(here("data","expr","Tabula_sapiens","ts.magma.aux.new.txt"), header = T, sep = "\t") %>% as_tibble()
ts_magma_ct_mapping <- read.table(here("data","expr","Tabula_sapiens","ts.fuma.aux.new.txt"), header = T, sep = "\t") %>% as_tibble()
...
write.table(t, file = here("results","Tabula_sapiens","ts_cor_methods.txt"), row.names = F, col.names = T, sep="\t")
