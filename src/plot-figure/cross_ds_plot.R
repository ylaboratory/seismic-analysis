...
droplet_cl_mapping <- read.table(here("results","Tabula_muris","droplet","droplet_ontology.txt"),sep="\t", header = T) %>% 
  ...
  filter(n()<2) %>%
  ungroup
ts_cl_mapping <- read.table(here("results","Tabula_sapiens","ts_ontology.new.txt"),sep="\t", header = T) %>% 
  as_tibble() %>%
  group_by(cell_ontology_id, tissue) %>%
  filter(n()<2) %>%
  ungroup
...
###load results
facs_res <- read.table(here("results","Tabula_muris","FACS","seismic","facs_res.txt"), sep="\t", header = T) %>% 
  as_tibble() %>%
  pivot_longer(!cell_type, names_to = "trait", values_to = "Pvalue")
droplet_res <- read.table(here("results","Tabula_muris","droplet","seismic","droplet_res.txt"), sep="\t", header = T) %>% 
  as_tibble() %>%
  pivot_longer(!cell_type, names_to = "trait", values_to = "Pvalue")
ts_res <- read.table(here("results","Tabula_sapiens","seismic","ts_res.new.txt"), sep="\t", header = T) %>% 
  as_tibble() %>%
  pivot_longer(!cell_type, names_to = "trait", values_to = "Pvalue")
