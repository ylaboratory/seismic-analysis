...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Input parameters are: anndata, gene set directory, result file name"
    )
    ...
    for gs_file in gs_file_all:
        gs_file = os.path.join(args.gene_set_dir, gs_file)
        gs = scdrs.util.load_gs(
            gs_file, src_species="hsapiens", dst_species="hsapiens", to_intersect=adata.var_names
        )
