...
def ass_mcz(rand_cell_idx, score_df, anndata):
    ...
    anndata.obs.iloc[
        rand_cell_idx, anndata.obs.columns.get_loc("cell_ontology_class")
    ] = "fake_cell_type"
    group_res = scdrs.method.downstream_group_analysis(
        adata=anndata, df_full_score=score_df, group_cols=["cell_ontology_class"]
    )
...
def comp_seed_table_ass_idx(seed_table, score_df, anndata, idx):
    ...
    return ass_mcz(seed_table.iloc[idx] - 1, score_df, anndata)  # 1-based, not 0-based
...
def main():
    """Main function to run the null simulation."""
    ...
    parser = argparse.ArgumentParser(
        description="Input parameters are: anndata, seed table, z-score file, number of cores and output files"
    )
    ...
    gs = scdrs.util.load_gs(
        args.gs_path, src_species="hsapiens", dst_species="mmusculus", to_intersect=adata.var_names
    )
    ...
    df_res = scdrs.score_cell(
        adata, gene_list, gene_weight=gene_weight, n_ctrl=1000,
