...
def prepare_adata(adata_obj, replace_mat, new_cell_anno, group = "cell_ontology_class"):
    ...
    missing_cell_ids = (set(perturbed_cell_id) - set(adata_obj.obs["cellid"])).union(set(renamed_cell_id) - set(perturbed_cell_id))
    if missing_cell_ids:
    ...
    #matrix replacement
    #row_indices = np.where(perturbed_adata.obs.index.isin(perturbed_cell_id))[0]
...
def get_causal_sim_res(adata_obj,  gene_set, group = "cell_ontology_class"):
    ...
    #score cells
    df_res = scdrs.score_cell(adata_obj, gene_list, gene_weight, n_ctrl=1000, return_ctrl_raw_score=False, return_ctrl_norm_score=True)
...
def scdrs_file_to_res(adata_file, replace_mat_file, cell_anno_file, gs_file, output_path, target_cell_type_pattern, extract_target_cell,group_to_replace = "new_cell_ontology_class", group = "cell_ontology_class"):
    ...
    if os.path.exists(output_path):
        ...
        gene_set = scdrs.util.load_gs(gs_file, src_species="hsapiens", dst_species="mmusculus", to_intersect=adata.var_names)
...
def parameter_df_to_res(parameter_df, output_header_col, target_cell_type_pattern, extract_target_cell, idx):
    ...
    res = scdrs_file_to_res(adata_file, replace_mat_file, cell_anno_file, gs_file, output_file_path, target_cell_type_pattern=target_cell_type_pattern, extract_target_cell=extract_target_cell)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Input parameters are: parameter data frame, output directory, organized_by column and number of cores")
    ...
    parser.add_argument('output_header_col', type=str, help='In the output directory, organize the output by the directories named after this column')
    parser.add_argument('target_ct_pattern', type=str, help='Regular expression to extract the target cell type')
    parser.add_argument('extract_target_cell', type=str, help='Extract the statistics of the target cell type or not. Any value that does not match any cell types will return 0.')
    ...
    f = partial(parameter_df_to_res, parameter_df, args.output_header_col, args.target_ct_pattern ,bool(args.extract_target_cell))
    
    with Pool(args.num_cores, initializer=worker_init) as p:
        asso_mcz_all = p.map(f, range(0,parameter_df.shape[0]))
    
    #with Pool(args.num_cores, initializer=worker_init, initargs=(0,)) as p:
    #   asso_mcz_all = p.starmap(f, [(i,) for i in range(parameter_df.shape[0])])
    
    dump(asso_mcz_all, args.final_res_path+"/scdrs_all_res.joblib")
    
    #create a data frame using asso_mcz_all, with column nam
