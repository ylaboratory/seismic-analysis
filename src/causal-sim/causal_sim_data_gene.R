...
sc3_gene_anno_dir <- here("data","gwas","causal_sim","sc3_standard","gene_anno")

sc3_cell_anno_dir <- here("data","expr","causal_sim","standard_scdesign3_model")

sc3_perturbed_expr_dir <- here("data","expr","causal_sim","sc3_standard","perturbed_expr")

##load fitted scDesign3 model
load(here("data","expr","causal_sim","standard_scdesign3_model","sc_para_list.rda"))
...
#get genes with certain expression and correct scdesign3 parameter
  ...
  map2(effect_size, names(effect_size), ~{
    ...
    map2(gene_anno_list, names(gene_anno_list), ~{
      ...
      map2(.x, names(.x), ~{
        ...
        coldata_df <-  cell_anno_tbl %>% as.data.frame() %>% set_rownames(.$cellid) %>% select(cell_ontology_class, library_size) #get cell metadata
        
        gene_list <- filter(.x, is_causal | is_ct_specific) %>% pull(mmu_symbol) #gene meta data
        
        cell_list <- cell_anno_tbl$cellid[cell_anno_tbl$is_target] #target cell type
         
        out_mat <- perturb_count_matrix(sce = sce, covar_df = coldata_df, cellids = cell_list, geneids = gene_list,  #perturbation
...
sc3_para_df <- expand_grid(gr_name = names(gene_anno), 
                           ...
                           expr_name = names(expr_list)) %>% #input parameter
...
sc3_multi_ct_gene_anno_dir_random <- here("data","gwas","causal_sim","sc3_multi_ct","random","gene_anno")
sc3_multi_ct_gene_anno_dir_no_overlap <- here("data","gwas","causal_sim","sc3_multi_ct","no_overlap","gene_anno")
sc3_multi_ct_gene_anno_dir_ct_overlap <- here("data","gwas","causal_sim","sc3_multi_ct","ct_overlap","gene_anno")
sc3_multi_ct_gene_anno_dir_causal_overlap <- here("data","gwas","causal_sim","sc3_multi_ct","causal_overlap","gene_anno")

sc3_multi_ct_cell_anno_dir <- here("data","expr","causal_sim","multi_ct_scdesign3_model")

sc3_multi_ct_perturbed_expr_dir_random <- here("data","expr","causal_sim","sc3_multi_ct","random","perturbed_expr")
sc3_multi_ct_perturbed_expr_dir_no_overlap <- here("data","expr","causal_sim","sc3_multi_ct","no_overlap","perturbed_expr")
sc3_multi_ct_perturbed_expr_dir_ct_overlap <- here("data","expr","causal_sim","sc3_multi_ct","ct_overlap","perturbed_expr")
sc3_multi_ct_perturbed_expr_dir_causal_overlap <- here("data","expr","causal_sim","sc3_multi_ct","causal_overlap","perturbed_expr")
...
##load fitted scDesign3 model
load(here("data", "expr", "causal_sim", "multi_ct_scdesign3_model", "sc_para_list.rda"))
